...
if [ -n "$VM_NODHCP" ] ; then
    ...
    cp /mnt/openstack-network /etc/sysconfig/network
    cp /mnt/openstack-ifcfg-eth0 /etc/sysconfig/network-scripts/ifcfg-eth0
...
if [ -n "$USE_PROVIDER_NETWORK" ] ; then
    ...
    openstack user create demo --project demo --password "$OPENSTACK_PASSWORD" --email demo@$VM_DOMAIN --enable
fi
